  ...
  for (chromosomeofstudy in 1:23) {
    ...
    while (j <= 2) {
      ...
      for (segmentofstudy in 1:segmentlengths[segmentindex]) {
        ...
        #Test in red
        plot(par_discretization, statofcurvesoutput[[3]], type="l", lwd = 3, col="red", ylim=c(0, 20), xlab = "Filtration Parameter", ylab = "Connected Components")
        # Control in blue
        ...
        title(paste("Betti 0 Curve", paste(chromosomeofstudy, armofstudy, "s", segmentofstudy, sep = ""), sep = " "))
        dev.off()
