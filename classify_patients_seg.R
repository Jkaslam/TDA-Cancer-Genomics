...
for (curve in curvetypes) {
  ...
  filename = paste("./DATA/", phenotype, " ", curve, " Non-Int Segments.csv", sep="")
  print(filename)
  if (!file.exists(filename)) {
    next
  }
  significant_segments <- read.csv(filename)
  print(filename)
  num_segments = nrow(significant_segments)
  if (phenotype == "HER2")  {
    phen_patients = which(horlings_phen$HER2==1)
  }
  else {
    if (phenotype == "ER") {
      phen_patients = which(horlings_phen$ER == 1)
    }
    else {
      phen_patients = which(horlings_phen$Molecular_Subtypes == phenotype)
    }
  }
  ...
  colnames(segment_classifications) = c("Segments", colnames(horlingsdata[,6:ncol(horlingsdata)]))
  ...
  path = paste("./DATA/LogisticData/", phenotype, curve, "segclass.csv", sep="")
